# 更新日志 | 项目全周期智能管理系统
## 2026-07-17
* 项目新增字段`进度填报周期`，可选`每周`或`每月`，留空表示该项目不启用进度跟踪
* 新增`项目进度报告`数据表和页面，按自然周（周一至周日）或自然月填报，每期只能填报一次
* 填报内容包括`完成百分比` `状态标签`（正常/预警/滞后）`本期进度` `下期计划` `问题与风险`
* 本期逾期未填报时，项目详情页显示提醒并提供`立即填报`入口
* `项目展示主页`和后台首页的项目卡片，对逾期未填报的项目标记`进度逾期`

## 2026-07-02
* 项目新增字段`项目简介`，展示于项目详情页图集右侧
* 新增项目逾期提醒：项目详情页显示逾期天数，项目列表在项目名称后标记`逾期 N 天`（已完成和已取消的项目不计入）
* 项目详情页`项目图集`为空时展示系统占位图
* `项目展示主页`轮播图同时展示项目基本信息
* 文档新增在线预览：支持 PDF、图片、文本、音频、视频和 Office 文档
* `项目任务`新增字段`所属阶段`，可关联至七大生命周期阶段
* 各生命周期阶段详情页新增该阶段的任务清单和完成情况统计
* 各生命周期阶段详情页新增`必传附件`清单，列出每类文档的上传状态、格式和大小要求，并提示必填附件是否齐全
* 七大生命周期阶段的名称、顺序、跳转和附件要求集中维护，各处展示保持一致
...
## 2026-06-24
* `项目展示主页`展示轮播图
* 新增页面`公司项目总览`，以树形结构显示组织架构及项目
* 项目新增字段：`封面图片` `项目图集` `监控`
* 项目详情页`封面图片` `项目图集` 以图片预览组件(lightbox)展示
* 项目详情页新增轮播图，展示该项目`项目图集`
* 现在点击`项目展示主页`和`项目详情页`中各节点，可跳转至相应页面/位置
* `负责人姓名`和`负责人电话`整合为`负责人`；`登记人姓名`和`登记人电话`整合为`登记人`
* `组织机构`新增字段`上级组织`，支持多级组织架构
* 数据可见范围按组织层级控制：用户可查看本组织及其下级组织的项目和各阶段流程；系统管理员不受限制

## 2026-05-10
* 新增`批量导入`：在项目列表上传 .xlsx 文件，一次登记多个项目，导入的项目状态为`草稿`
* 新增`下载导入模板`，模板中含各字段的填写说明和示例数据
* 导入时按行校验数据，错误集中提示；`项目类型`和`项目子类型`不存在时自动创建

## 2026-04-15
* 新增`系统设置`菜单，提供`修改密码`，修改时需验证旧密码
* 后台新建用户时若未填写密码，默认设置为 `111`
